...
A calibrated multi-agent assistant for distributed-trace root-cause localization.
Given one distributed trace, it returns a ranked top-3 of likely culprit spans, each
with a calibrated confidence and an evidence trail, or it abstains with "inconclusive
within budget". It is built as a controlled A/B/C experiment at an equal tool-call
budget, so "did multi-agent decomposition help, and did Tree-of-Thought on top of it
help" is a measured question rather than an assertion. CMU Agentic AI capstone.

The full writeup, results, and figures are in [`FINAL_REPORT.md`](FINAL_REPORT.md).
...
`REVIEW` (delivered but flagged), or `ESCALATE` (abstain, route to on-call SRE). An ESCALATE empties the
...
```bash
git clone https://github.com/diwanshu97/trace-reasoner.git && cd trace-reasoner
